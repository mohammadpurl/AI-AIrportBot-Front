...
export async function POST(req: NextRequest) {
  try {
    ...
    const target = `${base}/api/order/payment/link`;
    ...
    try {
      ...
      const payment_link = dataObj?.payment_link as string | undefined;
      const normalized = { status: Boolean(status), message, data: payment_link ? { payment_link } : undefined };
    ...
    return NextResponse.json({ status: false, message: e instanceof Error ? e.message : 'Proxy error' }, { status: 500 });
  }
}
