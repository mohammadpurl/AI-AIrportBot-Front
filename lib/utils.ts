...
// تابع تشخیص و تبدیل تاریخ (میلادی یا شمسی)
export function parseDate(dateString: string): DateObject | null {
  ...
  try {
    // تشخیص تاریخ میلادی (فرمت YYYY/MM/DD یا YYYY-MM-DD)
    if (/^\d{4}[\/\-]\d{1,2}[\/\-]\d{1,2}$/.test(normalizedDate)) {
      ...
      // اگر سال بین 1900 تا 2100 باشد، احتمالاً میلادی است
    ...
    // اگر تاریخ میلادی نبود، به عنوان شمسی پردازش کن
    return parsePersianDate(normalizedDate);
